...
UserRouter.post("/reserva", async (req, res) => {
    ...
    try{const reservaFind = await Reserva.find({$or: [
        {fechaSalida: { $gte: new Date(fechaSalida), $lte: new Date(fechaLlegada)}},
        {fechaLlegada: { $gte: new Date(fechaSalida), $lte: new Date(fechaLlegada)}}
    ]
    })
...
const user_id = await Properties.updateOne({_id: "60ee0706d460c340cc287fca"}, {disponibilidad: "asd" }, {upsert: true})
